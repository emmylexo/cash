...
if(isset($_POST['activate'])) {
    ...
    //Update user table
    $stm2 = $genInfo->runQuery("UPDATE orders 
      SET ord_status='1'
      WHERE payer_id=:uID");
    $stm2->execute(array(':uID'=>$uID));

    //Insert Into user notification table
    $action = 'The administrator has Activated your account!';
    $actionUrl = 'user/';
    ...
    //Insert Into admin notification table
    $action = $admInfo['username'].', has Activated user account';
    $actionUrl = 'customer-summary?uid='.$uID;
    $type = 'Account unrestricted';
    $username = $admInfo['username'];
    ...
    //include activate notifications
    $userID = $uID;
    include(ROOT_PATH."notifications/activate.php");

    $genInfo->redirect(BASE_URL.'administrator/customer-summary?uid='.$uID.'&activated');
    exit();
}
                            ...
                            <div class="col-lg-4">
                                <div class="panel panel-default" style="border:1px solid #f5f5f5;">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <?php }else if($uInfo["status"] == 'Blocked'){?>
                                            <form action="" method="post">
                                                <button name="unBlock" class="btn btn-warning btn-sm">Unblock User Account</button>
                                            </form>
                                        <?php } if(($uInfo["status"] == 'Pending') && ($orderInfo["ord_status"] == 2 )){?>
                                            <form action="" method="post">
                                                <button name="activate" class="btn btn-primary btn-sm">Activate User Account</button>
                                            </form>
                                        <?php }?>
                                        <br>
                                        <form action="" method="post">
