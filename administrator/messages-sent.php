<?php 
  ...
  $sentMessages = $adm->sentMessages();

  if(isset($_POST['msgID'])){
    $_SESSION['msgID'] = $_POST['msgID'];
    $genInfo->redirect(BASE_URL.'administrator/messages-sent?warning');
    exit();
  }
  ...
  if(isset($_GET['true']) AND isset($_SESSION['msgID'])) {
    foreach ($_SESSION['msgID'] as $msgID){
      try {      
        ...
        $genInfo->redirect(BASE_URL.'administrator/messages-sent?deleted');
        exit();      
      }
...
<?php if(isset($_GET['warning'])){?>
     <div class="alert alert-danger">
        <i class="fa fa-exclamation-triangle"></i> &nbsp; Are you sure you want to Delete? <a href="<?php echo BASE_URL.'administrator/messages-sent?true';?>">Yes</a>  | 
        <a href="<?php echo BASE_URL.'administrator/messages-sent';?>">No</a> 
     </div>
    ...
    <div class="table-responsive">
      ...
      <?php if(!empty($sentMessages)){
            foreach($sentMessages as $msg){
              $uInfo = $adm->userInfo($msg['receiver_id']);?>
      <tr>
