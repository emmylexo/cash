    ...
    <div class="row">
        <div class="col-sm-12">
            <ol class="breadcrumb">
              ...
              <li class="active">Get Merged Orders</li>
          </ol>
        </div>
    </div>
...
<?php if(!empty($orders)){?>
  ...
  <?php $i=0;
      ...
      <tr>  
        <td><?php echo $i; ?></td>
        <td><a target="_blank" style="color:#0099CC" href="<?php echo BASE_URL.'administrator/customer-summary?uid='.$payerInfo['login_id'];?>"><?php echo $payerInfo['first_name'].' '.substr($payerInfo['last_name'], 0, 1);?>.</a></td>

        <td><a target="_blank" style="color:#0099CC" href="<?php echo BASE_URL.'administrator/customer-summary?uid='.$payeeInfo['login_id'];?>"><?php echo $payeeInfo['first_name'].' '.substr($payeeInfo['last_name'], 0, 1);?>.</a></td> 

        <td><?php echo $defaultCurrency['c_symbol'].number_format((float)$order['ord_amount'], 2, '.', ',');?></td>

        <td><?php if($order['ord_status'] == 1){?>
            <span style="color: green">Paid</span>
            <?php }else{?>
            <span style="color:brown">Unpaid</span>
            <?php }?>
        </td>
        <td><?php echo strftime("%b %d, %Y %I:%M", strtotime($order['ord_date']));?></td>

        <td><a title="View History" href="<?php echo BASE_URL.'administrator/approve?ordid='.$order['ord_id'];?>" class="table-action-btn">
