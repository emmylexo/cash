<?php 
  ...
  if(isset($_GET['nid']) AND $_GET['nid'] != ''){
    ...
    $genInfo->redirect(BASE_URL.'administrator/news');
    exit();
  }

  $new = $adm->newSingle($id);
  if(empty($new)){
    $genInfo->redirect(BASE_URL.'administrator/news');
    exit();
  }
  ...
  if(isset($_POST['heading'])){
    ...
    try{
      $stmt = $genInfo->runQuery("UPDATE news
        SET subject=:heading, content=:content 
        WHERE id=:id");
      $stmt->execute(array(":id"=>$id, ":heading"=>$heading, ":content"=>$content));

      $genInfo->redirect(BASE_URL.'administrator/new?nid='.$id.'&updated');
      exit(); 
    }
                ...
                <?php
                  ...
                  <?php } }if(isset($_GET['updated'])){?>
                   <div class="alert alert-success">
                      <i class="fa fa-check-square"></i> &nbsp; News updated!
                   </div>
              ...
              <?php }?>
                <div class="col-lg-12">
                  <div class="card-box">
                    ...
                    value="<?php if(isset($new['subject'])){echo $new['subject'];}?>">
                        <br>
                    <textarea id="elm1" name="content"><?php if(isset($new['content'])){echo $new['content'];}?></textarea>
                  </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <hr />
                        <input type="submit"  class="btn w-sm btn-default"  value="Update">
                        </div>
                    </div>
                </div>
          </form>
