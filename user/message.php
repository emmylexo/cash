<?php 
  ...
  if(isset($_GET['msgid']) AND $_GET['msgid'] != ''){
    ...
    $genInfo->redirect(BASE_URL.'user/messages');
    exit();
  }

  $msg = $front->messageSingle($msgID);
  //
  $admInfo = $front->admInfo($msg['admin_id']);
  if(empty($msg)){
    $genInfo->redirect(BASE_URL.'user/messages');
    exit();
  }
  ...
  try {
    $stmt = $genInfo->runQuery("UPDATE messaging SET status=1
      WHERE msg_id=:msgID AND receiver_id=:loginID");
    $stmt->execute(array(':msgID'=>$msgID, ':loginID'=>$loginID));
  }
  ...
  if(isset($_POST['message'])) {
    ...
    if(!isset($error)){
      try {
        ...
        $stmt = $genInfo->runQuery("INSERT INTO messaging (msg_number, sender_id, subject, message, date_sent, attachment)

            VALUES (:msgNumber, :senderID, :subject, :message, :currentTime, :attachment)");
        ...
        $action = 'A message response from '.$userInfo['first_name'].' '.$userInfo['last_name'];
        $actionUrl = 'message?msgid='.$msgID;
        $type = 'User reply message';
        $username = '';
      ...
      $genInfo->redirect(BASE_URL.'user/message?msgid='.$msgID.'&message-sent');
      exit();
    }
  }
...
<div class="row">
<?php if($msg['sender_id'] != $loginID){?>
<form action="" method="post">
<div class="col-sm-12">
  <div class="media m-b-0">
    <a href="<?php echo BASE_URL.'user/dashboard';?>" class="pull-left">
      <?php if(file_exists(ROOT_PATH.str_replace('../', '', $userInfo['photo'])) AND $userInfo['photo'] != ''){?>
        <img alt="" src="<?php echo $userInfo['photo'];?>" class="media-object thumb-sm img-circle">
      <?php }else{?>
          <i style="font-size:25px;" class="ti-user"></i>
      <?php }?>
    </a>
