<?php 
  ...
  if(isset($_GET['phid']) AND $_GET['phid'] != ''){
    ...
    $genInfo->redirect(BASE_URL.'user/provide-help');
    exit();
  }
...
<?php if(!empty($orders)){?>
  ...
  <?php $i =0;
        foreach ($orders as $ph) {
          $i++;
          $payeeInfo = $front->userInfo($ph['payee_id']);?>
      <tr>  
        <td><?php echo $i;?></td>
        ...
        <td><a class="btn btn-default btn-sm" href="<?php echo BASE_URL.'user/payment?ordid='.$ph['ord_id'] ?>">View Details</a> </td>
      </tr>
      <?php }?>      
    </tbody>
  </table>
